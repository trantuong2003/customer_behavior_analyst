...
select top 5 item_purchased, round(avg(review_rating),2) as AverageReview from customer where review_rating > (SELECT AVG(review_rating) FROM customer) group by item_purchased order by AverageReview desc;
